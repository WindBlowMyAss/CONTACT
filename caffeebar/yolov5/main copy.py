...
def init_plugins(pluginRoot:np.str_="plugins") -> List[Dict]:
    """初始化插件
...
def run(weights=ROOT / 'yolov5l.pt',  # model.pt path(s)
    ...
    for path, im, im0s, vid_cap, _ in dataset:
        ...
        with dbm.open("/home/dbcloud/caffeebar/tmp.db", "n") as db:
            for k, v in sharedData.items():
                db[str(k)] = pickle.dumps(v)
        ###########################################################
        if view_img:
...
if __name__ == "__main__":
    opt = parse_opt()
    run(**vars(opt), plugins=init_plugins())
