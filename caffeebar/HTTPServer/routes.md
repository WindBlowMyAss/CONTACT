...
+ data
  ...
  + chairs元素
    ...
    | id        | `int` | 椅子的编号                                            |
    | desk       | `int` | 所属桌子的编号                 |
    | status | `str` | 状态；`available`: 可用，`busy`: 已占用，~~`miss`: 座位丢失~~ |

+ 示例

  ```json
  GET /api/det/chair?id=0
  
  {
  	...
  	"data": {
          "charis": [
              {"status": "available", "id": "1", "desk": "1"},
              {"status": "busy", "id": "2", "desk": "1"},
              // ...
          ]
      }
  }
